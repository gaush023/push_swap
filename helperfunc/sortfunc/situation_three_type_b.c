...
/*   Updated: 2023/10/25 13:20:09 by sagemura         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int bottom_a_value, int bottom_b_value)
{
	...
	{
		while (ft_lstsize(stack_a) > 3 && (*stack_a)->next->value > (*stack_b)->value
	...
	{
		if((*stack_a)->value > (*stack_a)->next->value && (*stack_a)->next->value <
		bottom_b_value )
			ft_sa(stack_a);
		ft_pb(stack_a, stack_b);
	}
